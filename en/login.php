...
if(isset($_POST['login_submit'])) {
    ...
    $email = mysqli_real_escape_string($mysqli_en, $email);
    $pass = $_POST['login_password'];
    $pass = mysqli_real_escape_string($mysqli_en, $pass);
        ...
        while ($row = mysqli_fetch_assoc($getHashAgent)) {
            ...
            if ((password_verify($pass, $hash))) {
                if($status == 1){
                    ...
                    $updateStatue = mysqli_query($mysqli_en,$queryUpdate);

                    $selectQuery = "SELECT * FROM `SUS_USERS` WHERE USER_ID = '{$id}'";
                    $runSelect = mysqli_query($mysqli_en,$selectQuery);
                    if(mysqli_num_rows($runSelect) > 0){
                        ...
                        if(date('Y-m-d H:i:s') < date("Y-m-d H:i:s", strtotime($time.'+2 hour'))){
                            ...
                            if($times > 1){
                                ...
                                $runSelectB = mysqli_query($mysqli_en,$selectB);
                                ...
                                $runDelete = mysqli_query($mysqli_en,$queryDelete);
                            }
                        }
                        ...
                        $runInsert = mysqli_query($mysqli_en,$queryInsert);
                    }
