...
if(isset($_POST['signup_submit'])) {
    ...
    $userName       = mysqli_real_escape_string($mysqli_en,$userName);

    $password   = $_POST['signup_password'];
    $password   = mysqli_real_escape_string($mysqli_en,$password);

    $passwordre   = $_POST['signup_password_re'];
    $passwordre   = mysqli_real_escape_string($mysqli_en,$passwordre);

    $fullName   = $_POST['signup_fullname'];
    $fullName   = mysqli_real_escape_string($mysqli_en,$fullName);

    $phone         = $_POST['signup_phone'];
    $phone         = mysqli_real_escape_string($mysqli_en,$phone);

    $birthday         = $_POST['signup_birthday'];
    $birthday         = mysqli_real_escape_string($mysqli_en,$birthday);

    $city        = $_POST['signup_city'];
    $city         = mysqli_real_escape_string($mysqli_en,$city);
    $date = date('Y-m-d');
    ...
    $getHashAgent = mysqli_query($mysqli_en, $query);
    ...
    <?php
        ...
        if(!empty($userName) && !empty($password) && !empty($passwordre)) {
            ...
            $insertUser = mysqli_query($mysqli_en_en, $query);
            if (!$insertUser) {
                die("Failed!" . mysqli_error($mysqli_en_en));
            } else {
                $insertUser =  mysqli_query($mysqli_en, $query);
                if (!$insertUser) {
                    die("FailedEN!" . mysqli_error($mysqli_en));
                }else{
                    $insertUser =  mysqli_query($mysqli_en_nl, $query);
                    if (!$insertUser) {
                        die("FailedNL!" . mysqli_error($mysqli_en_nl));
                    }else{
